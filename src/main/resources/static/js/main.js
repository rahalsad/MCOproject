(function () {
    ...
    bp = {
        dom: {
            ...
            selectTable: $('#PaimentChoixChekbox'),
            ...
            wrapperContent: $('.disconnected_content'),
            home_page: $('.home_page'),
            public_lang_btn: $('.public_lang_btn'),
            public_lang_list: $('.public_lang_list'),
            links_btn: $('.links_btn'),
            link_list: $('.link_list'),
            selectTableBourse: $('#BourseTableCheckbox'),
        ...
        datePicker : function(){
            var max_date= new Date();
               $( "#simulateur_date_naissance" ).datepicker({
                    maxDate: max_date,
                    inline: true,
                    changeMonth: true,
                    changeYear:true,
                    ...
                    onSelect: function (date) {
                        var date2 = new Date();
                        var start_date=$('#simulateur_date_naissance').datepicker('getDate');
                        var fin_date=$('#simulateur_date_naissance').datepicker('getDate');
                        var diffdate=diff_now("simulateur_date_naissance");
                        $("#input_age_assure").val(diffdate);
                        $("#age_assure").html(diffdate + " ans");

                        var start_date=$('#simulateur_date_naissance').datepicker('getDate');
                        var date_echeance = new Date(start_date);
                        var today=new Date();
                        var diffmonth;
                        var age_jouissance=$("#age_jouissance").val();
                        var input_type_simulation=$("#input_type_simulation").val();
                        console.log("input_type_simulation==>",input_type_simulation)
                        if(age_jouissance!=""){
                                date_echeance.setFullYear(date_echeance.getFullYear() + parseInt(age_jouissance));
                                $("#date_echeance_txt").html(date_echeance.getDate()+'/'+date_echeance.getMonth()+'/'+date_echeance.getFullYear());
                                months = (date_echeance.getFullYear() - today.getFullYear()) * 12;
                                months -= today.getMonth() + 1;
                                months += date_echeance.getMonth();
                                diffmonth= months <= 0 ? 0 : months;
                                duree_contrat=((date_echeance-today) / (31557600000));
                                $("#duree_contrat_txt").html(Math.round(duree_contrat +" ans"));
                                $("#input_duree_contrat").val(diffmonth);
                                if(input_type_simulation=="prestation"){
                                    bp.simulateEpargne("none","prestation");

                                }else{
                                    console.log("cotisation----------------")
                                    bp.simulateEpargne("none","cotisation");
              ...
              $('#datepicker3').datepicker('setDate',$.datepicker.regional["fr"]);
              $("#age_jouissance").on('change',function(){ // CALCUL AGE JOUISSANCE
                var start_date=$('#simulateur_date_naissance').datepicker('getDate');
                var date_echeance = new Date(start_date);
                var today=new Date();
                var diffmonth;
                ...
                if(start_date){
                date_echeance.setFullYear(date_echeance.getFullYear() + parseInt(this.value));
                $("#date_echeance_txt").html(date_echeance.getDate()+'/'+date_echeance.getMonth()+'/'+date_echeance.getFullYear());
                months = (date_echeance.getFullYear() - today.getFullYear()) * 12;
                months -= today.getMonth() + 1;
                months += date_echeance.getMonth();
        ...
        MultiEvolutionChart: function () {
            ...
            if ($('#evolution_chart_multi').length) {
                Highcharts.setOptions({
                    lang: {
                        shortMonths: ['Jan', 'Fev', 'Mar', 'Avr', 'Mai', 'Juin', 'Juil', 'Aout', 'Sept', 'Oct', 'Nov', 'DÃ©c'],
        ...
        costumDropDown: function () {
            ...
            bp.dom.greyDropDown.chosen({no_results_text: "Pas de r\351sultat pour ",search_contains:"true"});
        },
        ...
        publicLangSwitcher: function () {
            console.log('here')
            bp.dom.public_lang_btn.hover(function(){
            console.log('here publicLangSwitcher')
                bp.dom.public_lang_list.toggleClass('active');
                return false;
            });
        },
        ...
        simulatorsActions : function(){
            ...
            if ($('#slide_mensualite_ep').length) {
                $("#slide_mensualite_ep").slider().on("slide", function(slideEvt) {
                    // console.log($(".simulator_controll[data-target=" + $(this).attr('id') + "]"))
                    $(".simulator_controll[data-target=" + $(this).attr('id') + "]").val(slideEvt.value);

                    bp.simulateEpargne(slideEvt.value,"prestation");
                    var ratesVal = $('#input_mensualite_ep').val().replace('.',',');
                    $('#input_mensualite_ep').val(addSpace(ratesVal));
                ...
                $('#input_mensualite_ep').on('keyup', function(){
                    var val = parseFloat($(this).val().replace(/ /g,''))
                    $("#" + $(this).attr('data-target')).slider('setValue', val);
                    bp.simulateEpargne("none","prestation");
                })
            ...
            if ($('#slide_apport_ep').length) {
                $("#slide_apport_ep").slider().on("slide", function(slideEvt) {
                    // console.log($(".simulator_controll[data-target=" + $(this).attr('id') + "]"))
                    $(".simulator_controll[data-target=" + $(this).attr('id') + "]").val(slideEvt.value);

                    bp.simulateEpargne(slideEvt.value,"prestation");
                    var ratesVal = $('#input_apport_ep').val().replace('.',',');
                    $('#input_apport_ep').val(addSpace(ratesVal));
                }).on("slideStop", function(slideEvt) {
                    $(".simulator_controll[data-target=" + $(this).attr('id') + "]").val(slideEvt.value);
                    bp.simulateEpargne(slideEvt.value,"prestation");
                    var ratesVal = $('#input_apport_ep').val().replace('.',',');
                ...
                $('#input_apport_ep').on('keyup', function(){
                    var val = parseFloat($(this).val().replace(/ /g,''))
                    $("#" + $(this).attr('data-target')).slider('setValue', val);
                    bp.simulateEpargne(slideEvt.value,"prestation");
            ...
            if ($('#slide_taux_ep').length) {
                $("#slide_taux_ep").slider().on("slide", function(slideEvt) {
                    if (slideEvt.value < 10) {
                        slideEvt.value = "0" + slideEvt.value
                    }
                    $(".simulator_controll[data-target=" + $(this).attr('id') + "]").val(slideEvt.value);
                    ...
                    var ratesVal  = $('#input_taux_ep').val().replace('.',',');
                    $('#input_taux_ep').val(addSpace(ratesVal));
                }).on("slideStop", function(slideEvt) {
                    if (slideEvt.value < 10) {
                        slideEvt.value = "0" + slideEvt.value
                    }
                ...
                $('#input_taux_ep').on('keyup keypress', function(){
                    var val = parseFloat($(this).val().replace(/ /g,'').replace(/,/g,'.'));
                    if ($(this).val().indexOf(',') > 0) {
                        var numberFloat = ($(this).val().split(','));
                    }
                ...
                $('#input_taux_ep').on('keypress', function(){

                    return (event.charCode >= 48 && event.charCode <= 57)||( event.charCode == 44 )
                })
            };

            if ($('#slide_taux_epc').length) {
                $("#slide_taux_epc").slider().on("slide", function(slideEvt) {
                    if (slideEvt.value < 10) {
                        slideEvt.value = "0" + slideEvt.value
                    }
                    $(".simulator_controll[data-target=" + $(this).attr('id') + "]").val(slideEvt.value);
                    bp.simulateEpargne(slideEvt.value,"cotisation");

                    var ratesVal  = $('#input_taux_epc').val().replace('.',',');
                    $('#input_taux_epc').val(addSpace(ratesVal));
                }).on("slideStop", function(slideEvt) {
                    if (slideEvt.value < 10) {
                        slideEvt.value = "0" + slideEvt.value
                    }
                ...
                $('#input_taux_epc').on('keyup keypress', function(){
                    var val = parseFloat($(this).val().replace(/ /g,'').replace(/,/g,'.'));
                    if ($(this).val().indexOf(',') > 0) {
                        var numberFloat = ($(this).val().split(','));
                    }
                ...
                $('#input_taux_epc').on('keypress', function(){

                    return (event.charCode >= 48 && event.charCode <= 57)||( event.charCode == 44 )
                })
            };
            if ($('#slide_capital_rente').length) {
                $("#slide_capital_rente").slider().on("slide", function(slideEvt) {
                    // console.log($(".simulator_controll[data-target=" + $(this).attr('id') + "]"))
                    $(".simulator_controll[data-target=" + $(this).attr('id') + "]").val(slideEvt.value);

                    bp.simulateEpargne(slideEvt.value, "cotisation");
                    var ratesVal = $('#capital_rente').val().replace('.',',');
                    $('#capital_rente').val(addSpace(ratesVal));
                ...
                $('#capital_rente').on('keyup', function(){
                    var val = parseFloat($(this).val().replace(/ /g,''))
                    $("#" + $(this).attr('data-target')).slider('setValue', val);
                    bp.simulateEpargne(val, "cotisation");
                })

            };
            if($("#input_type_prestation").length){
                $("#input_type_prestation").on("change",function(){
                var val=parseFloat($("#capital_rente").val().replace(/ /g,''));
                bp.simulateEpargne(val, "cotisation");
            })
            };
        },
        ...
        simulateEpargne: function(slideVal,type){
            if(type=="prestation"){
            var mensualite = parseFloat($('#input_mensualite_ep').val().replace(/ /g,''));
            var taux= (parseFloat($('#input_taux_ep').val().replace(/ /g,'').replace(/,/g,'.')) / 100) + 1;
            var duree = parseFloat($('#input_duree_contrat').val().replace(/ /g,''));
            var apport=parseFloat($('#input_apport_ep').val().replace(/ /g,''));
            if (duree) {
                console.log("taux",taux)
                console.log("duree",duree)
                console.log("apport",apport)
                console.log("mensualite",mensualite)
                total_5=(mensualite*5*12*taux)+apport;
                total_10= (mensualite*10*12*taux)+apport;
                total_x= (mensualite*duree*taux)+apport;
                if(duree>60){
                    $(".total_5").show();
                    $(".libelle_total_5").show();
                    $(".total_5").html(addSpace(total_5.toFixed(2)) +" DHS"); //calcul après 5 ans
                } else{
                    $(".total_5").hide();
                    $(".libelle_total_5").hide();

                }
                if(duree>120){
                  $(".total_10").show();
                  $(".libelle_total_10").show();
                  $(".total_10").html(addSpace(total_10.toFixed(2)) +" DHS"); //calculer si duree de contrat est plus que 10 ans
              } else{
                  $(".total_10").hide();
                  $(".libelle_total_10").hide();

              }
                $(".libelle_total_x").html("A l'échéance: " + Math.round(duree/12) +" ans")
                $(".total_x").html(addSpace(total_x.toFixed(2)) +" DHS"); //calcul à l'échéance ans

                $(".rente_2").html(addSpace(Math.round(total_x/24).toFixed(2)) + " DHS");
                $(".rente_5").html(addSpace(Math.round(total_x/60).toFixed(2)) + " DHS");
                $(".rente_7").html(addSpace(Math.round(total_x/84).toFixed(2)) + " DHS");
                $(".rente_10").html(addSpace(Math.round(total_x/120).toFixed(2)) + " DHS");
             }

            }

            if(type=="cotisation"){
                var capital = parseFloat($('#capital_rente').val().replace(/ /g,''));
                var taux= (parseFloat($('#input_taux_epc').val().replace(/ /g,'').replace(/,/g,'.')) / 100) + 1;
                var duree = parseFloat($('#input_duree_contrat').val().replace(/ /g,''));
                var type_prestation = $('#input_type_prestation').val();

            if(duree!=0){
                switch (type_prestation){
                    case "capital":
                        console.log("taux",taux)
                        console.log("duree",duree)
                        console.log("capital",capital)
                        montant=(capital/duree)*taux; //Calcul de cotisation mensuelle
                        $(".input_cotis").val(addSpace(montant.toFixed(2)) + " " + type_prestation);
                        break;
                    case "rente_2":
                        montant=2*capital/duree;
                        $(".input_cotis").val(addSpace(montant.toFixed(2))+ " " + type_prestation);
                        break;

                    case "rente_5":
                        montant=5*capital/duree;
                        $(".input_cotis").val(addSpace(montant.toFixed(2))+ " " + type_prestation);
                        break;

                    case "rente_7":
                        montant=7*capital/duree;
                        $(".input_cotis").val(addSpace(montant.toFixed(2))+ " " + type_prestation);
                        break;
        ...
        contentScrollerHorsLigne : function() {

          if (bp.dom.wrapperContent.length) {
                console.log('here wrapperContent')
                var containerOffsetTop = bp.dom.wrapperContent.offset().top;
                var windowHeight = $(window).height()
                var containerHeight = windowHeight - containerOffsetTop;
                bp.dom.wrapperContent.css('height',containerHeight -20)
            };
        if (bp.dom.home_page.length) {
                console.log('here home_page')
                var containerOffsetTop = bp.dom.home_page.offset().top;
                var windowHeight = $(window).height()
                var containerHeight = windowHeight - containerOffsetTop;
                bp.dom.home_page.css('height',containerHeight -20)
            };


        /*    document.addEventListener('scroll',function(event){
                if(event.target.className==='second_content'){
        ...
        dataTableFunctions : function() {
                ...
                var myOperation = $('#myOperation').DataTable({
                        "bPaginate": false,
                        "aaSorting": [],
                            "aoColumnDefs": [
                ...
                var simulatorTablePratic = $('#simulatorTablePratic').DataTable( {
                    "bPaginate": false,
                "aaSorting": [],
                    "aoColumnDefs": [
                        { 'bSortable': false, 'aTargets': [0] },
            ...
            var BourseTableCheckbox = $('#BourseTableCheckbox').DataTable( {
                "bPaginate": false,
                "aaSorting": [],
                "aoColumnDefs": [
                    { 'bSortable': false, 'aTargets': [0] },
            ...
            var ObligationTableCheckbox = $('#ObligationTableCheckbox').DataTable( {
                "bPaginate": false,
                "aaSorting": [],
                "aoColumnDefs": [
                    { 'bSortable': false, 'aTargets': [0] },
            ...
            var bourse_historique = $('#bourse_historique').DataTable( {
                    "bPaginate": false,
                    "aaSorting": [],
                        "aoColumnDefs": [
                            { 'bSortable': false, 'aTargets': [0] },
            ...
            var bourse_detail_titre = $('#bourse_detail_titre').DataTable( {
                    "bPaginate": false,
                    "aaSorting": [],
                        "aoColumnDefs": [
                            { 'bSortable': true, 'aTargets': [0] },
        ...
        myselectTable : function(){
            ...
            //lot 3 ajouter les tableaux de bourse pour personnaliser les valeurs

            $("#BourseTableCheckbox td.select-checkbox").on('click', function(){
                $(this).parent().toggleClass('selected');
            });
            $("#ObligationTableCheckbox td.select-checkbox").on('click', function(){
                $(this).parent().toggleClass('selected');
            });
        },
        ...
        commandCarteActive : function(){
            $('#commander_btn').on('click', function(){
              /*  if ($(this).is(':checked')) {
                    //$('#commander_btn').removeAttr('disabled')
                }else{
                    //$('#commander_btn').attr('disabled', true)
                }*/
                console.log('here')
        ...
        accordion : function(){
                ...
                $('.bourse_title').on('click', function(){
                    if ($(this).hasClass('active')) {
                        $('.accordion_bourse').stop().slideUp(300);
                        $('.bourse_title').removeClass('active');
                    }else{
                        $('.accordion_bourse').stop().slideUp(300);
                        $(this).next('.accordion_bourse').stop().slideDown(300);
                        $('.bourse_title').removeClass('active');
                        $(this).addClass('active');
                    }
                    return false;
        ...
        backToUp : function(){
            ...
            $('.upToTop').on('click', function(){
                $('html, body, .content_container, .disconnected_content,  .home_page').stop().animate({ scrollTop: 0 },600);
            });

        },
        backToUpDis : function(){
            $('.disconnected_content').scroll(function(){
                console.log('scrollTop disconnected_content',$('.disconnected_content').scrollTop())
                if ($('.disconnected_content').scrollTop() > 100) {
                    $('.upToTop').fadeIn();
                } else {
                    $('.upToTop').fadeOut();
                }
        ...
        homePage : function(){ 
            ...
            $('.home_page').scroll(function(){ 
            console.log('scrollTop home_page',$('.home_page').scrollTop())
                if ($('.home_page').scrollTop() > 100) {
                    $('.upToTop').fadeIn();
                } else { 
                    $('.upToTop').fadeOut();
                }
        ...
        shoHideGeneralMenu : function(){
            ...
            $("#c-menu-left").on("click",function(){
                //$(this).hide();
                $('.menu').slideToggle("fast");        
                $('body').toggleClass("modal-open");

            });
            $("#c-menu-left c-menu__items").click(function(e) {
                e.stopPropagation();
           });
        },
        shoHideLinks : function(){
            bp.dom.links_btn.hover(function(){
        ...
        if ($('#table_agence').length) {
            ...
            $(function(){
                ...
                var LocsA = [
                    {
                        ...
                        html: '<div><img src="img/bp_ico_map.png" style="width:32px;padding-left:5px" ></div><b>SEIZE (16) NOVEMBRE</b> <br>Rue du 16 Novembre, Casablanca<br>0522-306195<br>0522-306201',
                        zoom: 12,
                    ...
                    },
                    {
                        ...
                        title: 'ZERKTOUNI',
                        html: '<div><img src="img/bp_ico_map.png" style="width:32px;padding-left:5px" ></div><b>ZERKTOUNI </b><br>101 Boulevard Mohamed Zerktouni, Casablanca<br>0522-306195<br>0522-306212',
                        zoom: 12,
                        icon: 'img/icon/marker_agence2.png'
                    },
                    {
                        lat: 33.6072578,
                        lon: -7.525583900000015,
                        title: 'HAJ OMAR ABDELJALIL',
                        html: '<div><img src="img/bp_ico_map.png" style="width:32px;padding-left:5px" ></div><b>HAJ OMAR ABDELJALIL </b><br>KM 7, 3 ROUTE DE RABAT AIN SEBAA<br>0522-306195<br>0522-306201<br>Centre Affaire',
                        zoom: 12,
                    ...
                    },
                    {
                        ...
                        title: 'LIBERTE',
                        html: '<div><img src="img/bp_ico_map.png" style="width:32px;padding-left:5px" ></div><b>LIBERTE </b><br>Boulevard de la Liberté Casablanca<br>0522-306195<br>0522-306201',
                        zoom: 12,
                        icon: 'img/icon/marker_agence2.png'
                    },
                ...
                for (var i = 0; i < LocsA.length; i++) {
                    var imageUrl = 'img/icon/marker_agence2.png';
                    switch (i){
                    case 0:
                    imageUrl= 'img/icon/marker_agence.png';
                    break;
                    case 1:
                    imageUrl= 'img/icon/marker_agence2.png';
                    break;
                    case 2:
                    imageUrl= 'img/icon/marker_center.png';
                    break;
                    } 
                ...
                $(".loc_link").on('click',function(){
                    ...
                    $(".onglets_list li:last-child a").addClass('active');
                    $(".onglets_list li:first-child a").removeClass('active');
                    $(".google_maps_agence").css("visibility","visible");
                    $("#gmap").css("height","500px");
                    google.maps.event.trigger(map, "resize");
                    $(".page_agence .padding_around").css("height","520px");                    
                    map.fitBounds(bounds);
        ...
        hideShowErrorContact : function(){
            ...
            $('input[name=general_terms]').on('change', function(){
                if ($('input[name=general_terms1]').is(":checked") || $('input[name=general_terms2]').is(":checked")){
                    $('input[name=general_terms1]').removeClass("has_error");
                    $('input[name=general_terms2]').addClass("has_error");
                    $(".error").hide()
                }
...
$(function() {
                  ...
                  simulatorTablePratic = $('#simulatorTablePratic').dataTable();
                  ...
                  BourseTableCheckbox = $('#BourseTableCheckbox').dataTable();
                  ObligationTableCheckbox = $('#ObligationTableCheckbox').dataTable();
                  ...
                  bourse_historique = $('#bourse_historique').dataTable();
                  bourse_detail_titre = $('#bourse_detail_titre').dataTable();
                })
                function filter_full_text(field_txt, i,target_table){ // valeur de champ, colonne cible, tableau cible
                    ...
                    switch (target_table){
                        ...
                        case "simulatorTable": // Page emprunt > Simulateur
                            ...
                            break;
                        case "simulatorTablePratic": // Page emprunt > Simulateur
                            simulatorTablePratic.fnFilter(text, i, false, true, false, true);
                            break;
                        ...
                        case "PaimentChoixChekbox":
                            ...
                            break;
                        case "BourseTableCheckbox": //personnalisation des valeurs table bourse
                            BourseTableCheckbox.fnFilter(text, i, false, true, false, true);
                            break;
                        case "ObligationTableCheckbox": //personnalisation des valeurs table obligation
                            ObligationTableCheckbox.fnFilter(text, i, false, true, false, true);
                            break;
                        case "favorisPaiment": // Page favoris paiement
                        ...
                        case "historiqueVirement": // Page compte > opération
                            ...
                            break;
                        case "misedisposition": // Page mise à disposition
                            misedisposition.fnFilter(text, i, false, true, false, true);
                            break;
                        case "historiqueTransfertMaroc": // Page transfert maroc 
                            historiqueTransfertMaroc.fnFilter(text, i, false, true, false, true);
                            break;
                        case "favorisTransfert": // Page favoris transfert
                        ...
                        case "table_agence": 
                            ...
                            break;
                        case "bourse_historique":
                            console.log("bourse_historique: ", text +' '+i)
                            bourse_historique.fnFilter(text, i, false, true, false, true);
                            break;bourse_detail_titre
                        case "bourse_detail_titre":
                            console.log("bourse_detail_titre: ", text +' '+i)
                            bourse_detail_titre.fnFilter(text, i, false, true, false, true);
                            break;
                ...
                if (theCheckBox.length>0)  {
                  ...
                  //filter in column 0, with an regex, no smart filtering, no inputbox,not case sensitive
                   ...
                   case "bourse_historique":
                   console.log('types',types +' '+ i)
                       bourse_historique.fnFilter(types, i, true, false, false, false);
                        break;
                   case "bourse_detail_titre":
                   console.log(' bourse_detail_titre types',types +' '+ i)

                       bourse_detail_titre.fnFilter(types, i, true, false, false, false);
                       break;
                    }
                ...
                case "PaimentChoixChekbox":
                    ...
                    break; //BourseTableCheckbox
                case "BourseTableCheckbox":
                    BourseTableCheckbox.fnSort([col, order]);
                    break;
                case "ObligationTableCheckbox":
                    ObligationTableCheckbox.fnSort([col, order]);
                    break;
                case "favorisPaiment":
                ...
                case "carnet_dordre":
                    ...
                    break;
                case "bourse_historique":
                    bourse_historique.fnSort([col, order]);
                    break;
                   case "bourse_detail_titre":
                    bourse_detail_titre.fnSort([col, order]);
                    break;
                }
...
function diff_now(from_picker_id) { // CALCULE LA DATE A PARTIR DES ID DES CHAMPS DATEPICKERS  
var from = $("#"+from_picker_id).datepicker('getDate');  
  var birthday = +new Date(from);
